...
class ResultObject extends AbstractObject {
    ...
    const INVALID_PARAMS_QUERY = -100011;
    ...
    const REGISTER_PRIVATE_SUCCESS =0;
    const REGISTER_PRIVATE_FAIL = 5;
    const AUTHORIZE_PRIVATE_SUCCESS = 0;
    const AUTHORIZE_PRIVATE_FAIL = 5;
    ...
    public function OutOfEncryptResponse() {
        ...
        //format data output
        $outputKeys = array("code", "desc", "data", "message");
        $data = $this->export();

        foreach ($outputKeys as $key => $value) {
            $ouputData[$value] = isset($data[$value]) ? $data[$value] : null;
        }
        echo Security::encrypt($ouputData, $this->getHashKey());
        die;
    }
