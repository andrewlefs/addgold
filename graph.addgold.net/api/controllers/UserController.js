...
module.exports = {
    authorize: function (req, res) {
        ...
        var temp = false;
        if(_.indexOf(params.account,"@") == 0){

            temp = true;
            params.account = params.account.slice(1);
        }
        ...
        function findAccount(callback){
            if(temp === true){
                params_filter.account_id = params.account;
            }else{
                params_filter.account = params.account;
            }

            Accounts.findOne(params_filter).then((result) => {
                if (_.isEmpty(result)) {
        ...
        function findMSIService(accountInfo, callback){
            if(temp === true){
                ...
                var today = new Date();
                var lasttemp = new Date(accountInfo.update_time_temp);

                if(accountInfo.update_time_temp == null || getMinutesBetweenDates(lasttemp,today) > 1){

                    callback("AUTHORIZE_FAIL");
                }

                if(accountInfo.password_temp != md5(params.password) ){
                    callback("AUTHORIZE_FAIL");
                }
        ...
        function createAccessToken(accountInfo, msiInfo, callback){
            ...
            if(accountInfo.status == 'lock'){
                ...
                AccessToken.create(dataToken).then((resultAccessToken) => {
                    ...
                    var keyCache = redisKey.getKeyAccessToken(md5(resultAccessToken.access_token));
                    redisCache.set(keyCache,  {
    ...
    authorize_apple: function (req, res) {
        var params = req.requestParams;
        //params.publicKeyUrl = "https://static.gc.apple.com/public-key/gc-prod-3.cer";
        var needles = ['playerID','publicKeyUrl','salt','signature','bundleId','timestamp'];
        ...
        var identity = {
            publicKeyUrl: params.publicKeyUrl,
        ...
        verifier.verify(identity, function (err, token) {
            if (!err) {
                ...
                //console.log(token);
                //return res.toJSON('AUTHORIZE_APPLE_SUCCESS', token);
                async.waterfall([
                    function(callback) {
                        findsocialApple(callback);
                    },
                    function(accountInfo, callback) {
                        findMSIService(accountInfo, callback);
                    },
                    ...
                    if(err){
                        ...
                        return res.toJSON('AUTHORIZE_APPLE_SUCCESS', tokenInfo);
                    }
                });


                // FIND socialGoogle
                function findsocialApple(callback){
                    SocialApple.findOne({playerId: token.playerId}).then((result) => {
                        if (_.isEmpty(result)) {
                            params.type = 'apple';
                            params.ios_ifa = (!_.isUndefined(req.trackingInfo.ios_ifa) && !_.isEmpty(req.trackingInfo.ios_ifa)) ? req.trackingInfo.ios_ifa : '';
                            Accounts.create(params).then((accountInfo) => {
                                console.log(accountInfo);
                                SocialApple.create({
                                    account_id: accountInfo.account_id,
                ...
                return res.toJSON('AUTHORIZE_APPLE_FAIL');
            }
        });
    ...
    register_quickly: function (req, res) {
        ...
        console.log('DEBUG REGISTER QUICLY');
    ...
    sso: function (req, res) {
        ...
        // 2017-08-31 - VietBL: fix bug auto login thanh cong ngay, bo sung require access_token
        var needles = ['device_id', 'access_token'];
    ...
    check_email_api: function (req, res) {
        var params = req.requestParams;

        var email_user = _.toLower('vochitaidt@gmail.com');
        Accounts.findOne({email: email_user}).then((accountInfo) => {
            if (_.isEmpty(accountInfo) == false) {
                return res.toJSON('ACCOUNT_EXIST', {
                    account_id: accountInfo.account_id,
                    email: accountInfo.email
                ...
                return res.toJSON('ACCOUNT_NOT_EXIST',{},true);
            }
        });
    },

    register_mu: function (req, res) {

        var params = req.allParams();

        console.log("REGISTER_MU");
        var needles = ['user_name', 'password', 'email'];
        ...
        if (validateRequest.required(params, needles,false) == false) {
            return res.toJSON('INVALID_PARAMS',{},true);
        }

        if (validateRequest.validAccount(params.user_name) == false) {
            return res.toJSON('ACCOUNT_NOT_VALID',{},true);
        }
        ...
        async.waterfall([
            ...
            function(callback) {
                findAccount({account: params.user_name}, function(err, result){
                    if(err == 'EMPTY'){
                        callback(null);
                    }
                    else{
                ...
                activityService.verify(req, res,
                    {
                        ...
                        if(err){
                            return res.toJSON('SERVER_MAINTENANCE',{},true);
                        }
                        else{
                            return res.toJSON('REGISTER_PRIVATE_SUCCESS', tokenInfo,true);
                        }
                    });
        ...
        function findAccount(where, callback){
            Accounts.findOne(where).then((result) => {
                ...
                callback('REGISTER_PRIVATE_FAIL');
            });
        }
        ...
        function createAccount(callback){
            ...
            Accounts.create(params).then((result) => {
                ...
                params.model = (!_.isNull(params.model) && !_.isUndefined(params.model)) ? params.model : '';
                var ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
                var key_anti = 'ANTI_CHEAT_HACK_CREATE_ACCOUNT' + ip + params.platform + params.model;
                redis.set(key_anti, true, 'EX', 300);
        ...
        function createAccessToken(accountInfo, msiInfo, callback){
            ...
            AccessToken.create(dataToken).then((resultAccessToken) => {
                ...
                var tokenInfo = {
                    ...
                    KL_SSO: resultAccessToken.access_token,
                    KL_PERSON: resultAccessToken.access_token,
    ...
    authorize_mu: function (req, res) {
        var params = req.allParams();
        var needles = ['username', 'userpass'];
        ...
        params.account = _.toLower(params.username);
        ...
        async.waterfall([
            ...
            if(err){
                ...
                return res.toJSON('AUTHORIZE_PRIVATE_SUCCESS', tokenInfo,true);
            }
        });
        ...
        function findAccount(callback){
            ...
            params_filter.is_private = "true";

            Accounts.findOne(params_filter).then((result) => {

                if (_.isEmpty(result)) {
                    callback('AUTHORIZE_PRIVATE_FAIL');
                } else {
                    callback(null, result);
                }
            }, error => {
        ...
        function findMSIService(accountInfo, callback){
            if(temp === true){
                ...
                if(accountInfo.password_temp != md5(params.userpass) ){
                    callback("AUTHORIZE_PRIVATE_FAIL");
                }

            }else{
                if (accountInfo.password != md5(params.userpass)) {
                    callback("AUTHORIZE_PRIVATE_FAIL");
                }
            }
    ...
    verify_access_token_mu: function (req, res) {
        var params = req.allParams();
        var needles = ['access_token'];
        ...
        async.waterfall([
            ...
            if (err && err != 'REDIS') {
                ...
                params.account_id = accountInfo.uid;
        ...
        function findAccessToken(callback) {
            ...
            AccessToken.findOne({hash: md5(params.access_token)}).then((result) => {
                if (_.isEmpty(result)) {
                    callback("VERIFY_ACCESS_TOKEN_PRIVATE_FAIL");
                } else {
                    callback(null, result);
                }
            }, error => {
        ...
        function findAccount(msiInfo, callback) {
            Accounts.findOne({account_id: msiInfo.account_id}).then((result) => {
                if (_.isEmpty(result) == true) {
                    ...
                    var tokenInfo = {
                        uid: msiInfo.account_id,
                        KL_SSO: params.access_token,
                        KL_PERSON: params.access_token,
    ...
    change_password_mu: function (req, res) {
        var params = req.requestParams;
        var needles = ['new_password', 'password', 'user_name'];
        ...
        async.waterfall([
            ...
            if(err){
                ...
                return res.toJSON('CHANGE_PASSWORD_PRIVATE_SUCCESS');
            }
        });

        // FIND Account
        function findAccount(msiInfo, callback){
            Accounts.findOne({account: params.user_name}).then((result) => {
                if (_.isEmpty(result) == true) {
                    callback('CHANGE_PASSWORD_PRIVATE_FAIL');
                } else {
