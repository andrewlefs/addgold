...
/**
 * Description of Service_10000: APIs cho game Game of Kings
 *
 * @author vietbl
 */
...
class Service_10000 {
    ...
    private $test_domain = 'http://203.162.80.92'; // for testing
    // BEGIN REAL SERVER
    private $real_domain = 'http://103.57.220.28'; // for prod
    private $api_payment = '/payway/payment/cok/notify_url.php';
    private $api_role_info = ''; // truy vấn DB, xem KingofGameModel
    private $api_add_item = '/payway/payment/cok/notify_url.php'; // dùng luôn API add gold
    private $api_substract_item = '';
    private $api_topdata = '';
    private $api_ccu = '';
    private $api_kick_user = '';
    private $api_secret = '5xx1361uizu5an351nfp';
    ...
    private $CI;
    private $err_msg;
    private $payment_rate = 0.05; //
    private $payment_unit = 'Vàng';
    private $card_type_default = '';
    private $arr_card_type_promo = array
        (
        //array("min_pcoin" => 100000, "max_pcoin" => 110000, "card_type" => 1, "is_count" => true, "count" => 1, "desc" => array(1 => "Gói 1000 Vàng")),
        //array("min_pcoin" => 20000, "max_pcoin" => 22000, "card_type" => 2, "is_count" => false, "count" => 0, "desc" => array(1 => "Gói 200 vàng không giới hạn trong ngày")),
        //array("min_pcoin" => 50000, "max_pcoin" => 66000, "card_type" => 3, "is_count" => false, "count" => 0, "desc" => array(1 => "Gói 500 vàng không giới hạn trong ngày")),
        //array("min_pcoin" => 200000, "max_pcoin" => 220000, "card_type" => 4, "is_count" => true, "count" => 1, "desc" => array(1 => "Gói KM 1200 Vàng")),
        //array("min_pcoin" => 100000, "max_pcoin" => 110000, "card_type" => 5, "is_count" => true, "count" => 1, "desc" => array(1 => "Gói KM 500 Vàng")),
        //array("min_pcoin" => 300000, "max_pcoin" => 330000, "card_type" => 6, "is_count" => true, "count" => 1, "desc" => array(1 => "Gói KM 1800 Vàng")),
        //array("min_pcoin" => 500000, "max_pcoin" => 550000, "card_type" => 7, "is_count" => true, "count" => 1, "desc" => array(1 => "Gói KM 4600 Vàng"))
    );
    ...
    public function recharge(obj_service $service, obj_distribution $distribution, obj_tracking $tracking, obj_game_info $obj_game_info, obj_payment_recharge $obj_payment_recharge, $params, $data = null) {
        ...
        if ($promoCount > 0) {
            ...
            for ($idx = 0; $idx < $promoCount; $idx++) {
                sleep(1);
                $data['order_id'] = "promo_" . $cloneOrderId . "_" . $idx;
                $original_data = $data['account_id'] . $data['order_id'] . $data['time_stamp'] . $data['pay_way'] . $data['pay_amount'] . $data['final_get_money'] . $data['original_get_money'] . $data['server_id'] . $this->api_secret;
                //echo $original_data;
                //echo '-------';
                $data['sign'] = md5($original_data);
     ...
     * http://gapi.addgold.net/?control=game&func=add_item&account_id=989411371&server_id=1&service_name=10000&service_id=10000&amount_vnd=10000&time_stamp=2015-05-05 08:00:13&award=[{"item_id":1,"count":1}]&title=Test thử add vàng&content=Test thử add vàng&app=xxx&token=xxx
     */

    public function add_item($params) {
        $needle = array('account_id', 'character_id', 'server_id', 'service_name', 'service_id', 'awards');
        if (is_required($params, $needle) == FALSE) {
        ...
        $params['character_id'] = $user_info[0]['gameUid'];
        ...
        $idInserted = $this->CI->GameModel->set_event_item($params['service_name'], $params['account_id'], $params['transaction_id'], $params['character_id'], $params['character_name'], $params['server_id'], 0, 0, $params['time_stamp'], $params['awards'], $params['title'], __FUNCTION__, $params['full_request']);
        if ($idInserted == null)
            return array('status' => false, 'message' => 'INSERT_EVENT_TRANSACTION_FAIL', 'desc' => 'INSERT_EVENT_TRANSACTION_FAIL');

        $awards = json_decode($params['awards'], true);
        ...
        foreach ($awards as $key => $value) {
            ...
            for ($idx = 0; $idx < $count; $idx++) {
                ...
                $data['final_get_money'] = ($pcoin * $this->payment_rate);
                $data['original_get_money'] = ($pcoin * $this->payment_rate);
