...
class BTCeAPI {
    ...
    public function apiQuery($method, $req = array()) {
        ...
        if (!$result) {
			...
			$result['msg'] = 'Invalid data received, please make sure connection is working and requested API exists';
            return $result;
        }
        ...
        if (isset($result['error']) === true) {
            if (strpos($result['error'], 'nonce') > -1 && $this->RETRY_FLAG === false) {
                ...
                $result['msg'] = 'API Error Message: ' . $result['error'] . ". Response: ";
				//.json_encode($result);
				//. print_r($result, true);
     ...
     * @return type
     */
    public function createCoupon($currency, $amount) {
